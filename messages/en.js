exports.label = 'En';
exports.trans = {

	section_about: 'About us',
	section_forwhom: 'For whom',
	section_services: 'Services and prices',
	section_portfolio: 'Portfolio',
	section_reviews: 'Reviews',
	section_questions: 'Questions',
	section_contacts: 'Contacts',
	
	// HOME
	home_web_development: 'Web Development',
	home_wanna_website: 'Wanna website?',
	home_order_now: 'Order Now!',
	
	// ABOUT US
	about_us_1:
		'<p>\
			Мы - небольшая KOKO специалистов-фрилансеров в областивеб-разработки. Мы консультируем по вопросам развработкивеб-сайтов,\
			советуем наиболее подходящие решения. Разрабатываемlanding page, адаптивные многостраничные сайты и интернет-магазины,основаясь\
			на автоматизации бизнес-процессов. Нам очень важно,чтобы каждый созданный нами сайт имел высокое качество,так как сайт\
			не только представляет клиента, но так же являетсянашей рекламой.\
		</p>',
	about_us_2:
		'<h3>Преимущества работы с нами</h3>\
		<ul class="fa-xul">\
			<li>Мы предосталяем бесплатнуе консультации.</li>\
			<li>Отсутствие выплат налогов, как в случае со штатным сотрудником, а следовально цены на услуги выгоднее.</li>\
			<li>Нам не важна география, Вам не обязательно находится в Латвии,чтобы мы могли помочь Вам.</li>\
			<li>Договорные отношения с четко прописанными обязанностями и ответственностью Сторон.</li>\
			<li>Высокая скорость исполнения задачь</li>\
			<li>Онлайн-поддержка 24/7</li>\
			<li>В разработке каждого нового проекта и в поддержке существующихмы всегда думаем о том, какую пользу получит ваш клиентот взаимодействия с сайтом, от оптимизации процессов,от персонифицированных коммуникаций!</li>\
		</ul>',
		
	// FOR WHOM
	for_whom_title: 'Кому подойдет наше предложение?',
	for_whom_text: '\
		<li>Для тех, кто хочет выйти на новый уровень прибыльности бизнеса.</li>\
		<li>Сформировать клиентску базу.</li>\
		<li>Получить целый арсенал для мотивации клиентов.</li>\
		<li>Тем, кто хочет автоматизировать рутинные задачи.</li>\
		<li>Тем, кто ценит своё время. </li>\
		<li>Тем, кто хочет добиться большей эффективности отдела продаж. </li>\
	',
		
	// SERVICES
	services_1_title: 'Сайт визитка',
	services_1_text: '\
		<li>Lorem ipsum dolor sit amet,</li>\
		<li>consectetur adipiscing elit.</li>\
		<li>Vivamus at augue imperdiet</li>\
	',
	services_2_title: 'Корпоративный сайт',
	services_2_text: '\
		<li>Lorem ipsum dolor sit amet,</li>\
		<li>consectetur adipiscing elit.</li>\
		<li>Vivamus at augue imperdiet</li>\
	',
	services_3_title: 'Интернет магазин',
	services_3_text: '\
		<li>Lorem ipsum dolor sit amet,</li>\
		<li>consectetur adipiscing elit.</li>\
		<li>Vivamus at augue imperdiet</li>\
	',
	services_order_now: 'Order Now',
		
	// QUESTIONS
	questions: '\
		<div>\
			<h2>вопрос блабла ?</h2>\
			<p>Lorem ipsum dolor sit amet,consectetur adipiscing elit. Vivamus at augue imperdietor sit amet,consectetur adipiscing elit. Vivamus at augue imperdietor siterdiet</p>\
		</div>\
		<div>\
			<h2>вопрос блабла ?</h2>\
			<p>Lorem ipsum dolor sit amet,consectetur adipiscing elit. Vivamus at augue imperdietor sit amet,consectetur adipiscing elit. Vivamus at augue imperdietor siterdiet</p>\
		</div>\
		<div>\
			<h2>вопрос блабла ?</h2>\
			<p>Lorem ipsum dolor sit amet,consectetur adipiscing elit. Vivamus at augue imperdietor sit amet,consectetur adipiscing elit. Vivamus at augue imperdietor siterdiet</p>\
		</div>\
	',

	// CONTACTS
	contacts_name: 'Имя',
	contacts_phone: 'Телефон',
	contacts_email: 'E-Mail',
	contacts_landing: 'Визитка',
	contacts_corporate: 'Корпоративный',
	contacts_ecommerce: 'Интернет магазин',
	contacts_comments: 'Дополнительная информация',
	contacts_send: 'Отправить',
	contacts_mailresult_success: 'Спасибо, форма отправлена, свяжемся в ближайшее время!',
	contacts_mailresult_error: 'Ошибка отправки формы, напишите нам сами на <a href="mailto:${email}">${email}</a>',
	
	// footer
	all_rights_reserved: 'All rights reserved 2018',
};
